...
describe('isNumberOrDecimal()', () => {
	test('contains a string, must return an error', async(done) => {
		expect.assertions(1)
		const result = utilValidator.isNumberOrDecimal('string')
		const expectedResult = { valid: false, error: 'Must be a number or a decimal'}
		expect(result).toEqual(expectedResult)
		done()
	})
	test('contains a number, must return a success validation', async(done) => {
		expect.assertions(1)
		const result = utilValidator.isNumberOrDecimal(1234)
		const expectedResult = { valid: true}
		expect(result).toEqual(expectedResult)
		done()
	})
	test('contains a decimal, must return a success validation', async(done) => {
		expect.assertions(1)
		const result = utilValidator.isNumberOrDecimal(3.5)
		const expectedResult = { valid: true}
		expect(result).toEqual(expectedResult)
		done()
	})
